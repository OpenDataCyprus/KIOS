...
void DijkstraTime(int source, int numNodes, int num_of_edges,int n,double **matrix,int *prev,long double *d, int* visited, double duration, double speed, int difLevel, int *dest, double **adjList){
	int i, k, mini;
	double currentLaneCost=0.0;
	double distToCover=duration/2;
	int elevationPos;
        ...
        for (i = 0; i <= n; ++i){
            if (matrix[mini][i]!=INFINITE){
				...
				if(difLevel==1){ // expert
					 ...
					 currentLaneCost=(matrix[mini][i]/speed)-5*adjList[elevationPos][3]+100;

				}else{
									
					if(difLevel==2){ // Intermidiate
							
					 currentLaneCost=(matrix[mini][i]/speed)+1*adjList[elevationPos][3]+100;
